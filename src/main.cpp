...
uint256 hashGenesisBlock("0x00000a97cc51c61ef0212cd2113603c96c72ea9e1a30e72d15da1297890c64d3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // bowscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "bowscoin Signed Message:\n";
...
{
    // bowscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 150; // bowscoin: 2.5 minute
static const int64 nTargetSpacing = 150; // 2.5 minute
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // bowscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // bowscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "StartUP!!! - CEO of Mining Company on The Future of Bitcoin 05/05/2015 - Bowscoin Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = nSubsidyGenesisBlock;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("034a60d3723f4cafe4e27d347277f4fc516c5eb7846eb90ae47dccdeb1a440f470") << OP_CHECKSIG;
        ...
        block.nVersion = 1;
        block.nTime    = 1430817488;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 18259561;
        ...
        // Merkle root
        assert(block.hashMerkleRoot == uint256("0x6d6ea88f8b6542a0e06c25c7ddbfe99ebcad7101615a84d7328851e3d22a756d"));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, pchMessageStart, sizeof(pchMessageStart)) != 0) {
            printf("\n\nPROCESSMESSAGE: INVALID MESSAGEBSC\n\n");
            fOk = false;
            break;
        }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("bowscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("bowscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static bowscoinMiner(CWallet *pwallet)
{
    printf("bowscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bowscoin-miner");
    ...
    try { loop {
        ...
        printf("Running bowscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("bowscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&bowscoinMiner, pwallet));
}
